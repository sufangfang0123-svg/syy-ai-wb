...
function PublicContentPage() {
  ...
  return <div className="page-frame">
    ...
    <div className="content-studio-layout">
      ...
      <aside className="content-review panel-surface"><p className="section-kicker">Review & trace</p><h2 className="section-title">审核与追溯</h2><div className="trace-chips">{active.evidenceIds.map((id) => <Link key={id} href="/evidence">Evidence {id}</Link>)}<Link href="/evolution">概念 {active.conceptId}</Link></div><div className="risk-stack mt-4"><p><ShieldAlert className="h-4 w-4" />{active.complianceRisk}</p></div><label className="form-field mt-4"><span>审核意见（必填）</span><textarea value={reviewNote} onChange={(event) => setReviewNote(event.target.value)} aria-describedby="content-review-reason" /></label><p id="content-review-reason" className="disabled-reason">选择任何审核状态前都必须记录人工理由；公开版只写入模拟审计。</p><div className="review-actions">{(["pending", "approved", "changes", "rejected"] as ReviewStatus[]).map((status) => <button key={status} disabled={!reviewNote.trim()} aria-describedby="content-review-reason" className={active.reviewStatus === status ? "active" : ""} onClick={() => { reviewContent(active.id, status, reviewNote); setMessage(`已记录${reviewLabels[status]}及审核理由`); }}>{status === "approved" ? <Check className="h-3.5 w-3.5" /> : status === "changes" ? <FileEdit className="h-3.5 w-3.5" /> : status === "rejected" ? "×" : "·"}{reviewLabels[status]}</button>)}</div><div className="content-lineage"><GitCompareArrows className="h-4 w-4" /><p>反馈可从<Link href="/results">转化反馈</Link>回到此版本；概念实质变更时本版本会显示 stale。</p></div></aside>
    </div>
  </div>;
}
