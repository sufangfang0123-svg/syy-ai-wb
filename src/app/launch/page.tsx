...
function PublicLaunchPage() {
  ...
  return <div className="page-frame">
    ...
    <section className="scenario-table" aria-label="100个未评分候选情景"><header><span>情景</span><span>人群 / 卖点</span><span>渠道 / CTA</span><span>评分状态</span><span>人工状态与操作</span></header>{visible.map((item) => { const review = reviewMap.get(item.id); const canRecord=Boolean(owner.trim()&&note.trim()); const selectedForShortlist=shortlistSelections.includes(item.id); return <article key={item.id} className={review?.stale ? "stale" : ""}><label><input type="checkbox" checked={selected.includes(item.id)} onChange={() => toggleCompare(item.id)} aria-label={`对比${item.id}`} /><span>{item.id}</span><small>{item.assumptionId}</small></label><div><strong>{item.persona}</strong><small>{item.sellingPoint}</small></div><div><strong>{item.channel}</strong><small>{item.cta}</small></div><div className="priority-cell" title={item.rule}><b>未评分</b><small>缺少情景级输入</small></div><div><span className={`review-pill review-${review?.status ?? "pending"}`}>{review?.stale ? "stale" : review?.status === "shortlisted" ? "人工 shortlist" : review?.status === "validation" ? "已进入验证" : "未复核"}</span>{!review||review.status==="unreviewed"?<label className="shortlist-check"><input type="checkbox" checked={selectedForShortlist} onChange={()=>toggleShortlist(item.id)} aria-label={`选择${item.id}加入人工shortlist`}/><span>明确选择</span></label>:null}<button disabled={!canRecord||!selectedForShortlist||Boolean(review&&review.status!=="unreviewed")} onClick={() => {reviewScenario(item.id, "shortlisted", owner.trim(), note.trim());setShortlistSelections((current)=>current.filter((id)=>id!==item.id));}}>加入人工 shortlist</button><button disabled={!canRecord||review?.status!=="shortlisted"} onClick={() => reviewScenario(item.id, "validation", owner.trim(), note.trim())}>标记进入验证</button></div></article>; })}</section>
    <section className="panel-surface mt-6"><div className="panel-title-row"><div><p className="section-kicker">Human review</p><h2>人工筛选理由与责任人</h2></div><span className="meta-chip">不会自动排序或形成shortlist</span></div><div className="form-grid mt-4"><label className="form-field"><span>负责人（模拟自述）</span><input value={owner} onChange={(event) => setOwner(event.target.value)} /></label><label className="form-field"><span>筛选理由（必填）</span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="记录为什么把某个情景加入人工shortlist" /></label></div><p className="mt-3 text-xs text-[#6F7D77]">固定演示Evidence引用：{selectedConcept.evidenceIds.join(" · ") || "无"}。操作、理由、时间与这些固定演示引用会保存在模拟审计中。</p><div className="mt-4 flex flex-wrap gap-3"><Link href="/tests" className="primary-action">查看既有Validation记录<ArrowRight className="h-4 w-4" /></Link><Link href="/evidence" className="secondary-action"><FlaskConical className="h-4 w-4" />进入预验证Gate</Link></div><p className="mt-3 text-xs text-[#6F7D77]">公开演示只记录模拟人工筛选；真实项目必须在本地工作区引用已确认Evidence并创建Validation，未确认内容不影响Gate。</p></section>
  </div>;
}
