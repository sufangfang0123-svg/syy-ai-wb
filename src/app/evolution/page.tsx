...
function PublicEvolutionPage() {
  ...
  return <div className="page-frame">
    ...
    <section className="panel-surface mt-6"><div className="panel-title-row"><div><p className="section-kicker">Human selection</p><h2>选择理由与版本锁定</h2></div><span className="simulation-chip">{selectedConcept.id} · V{selectedConcept.version}</span></div><label className="form-field mt-4"><span>人工选择理由（必填）</span><textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="记录取舍依据、反证和仍需验证的问题" aria-describedby="concept-selection-reason" /></label><p id="concept-selection-reason" className="disabled-reason">未记录人工理由时不能选择或锁定概念。</p><div className="mt-4 flex flex-wrap gap-3"><button className="primary-action" disabled={!reason.trim()} aria-describedby="concept-selection-reason" onClick={() => { selectConcept(selectedConcept.id, reason); lockConcept(selectedConcept.id); }}><Lock className="h-4 w-4" />确认并锁定当前版本</button><Link href="/launch" className="secondary-action">进入情景筛选<ArrowRight className="h-4 w-4" /></Link></div></section>
  </div>;
}
