...
export default function ProductSite() {
  ...
  return <div>
    <section className="hero-section"><div className="mx-auto grid max-w-[1360px] items-center gap-10 px-4 py-14 sm:px-6 md:py-20 lg:grid-cols-[1.08fr_.92fr] lg:px-8 lg:py-24">
      <div><div className="flex flex-wrap items-center gap-3"><span className="enterprise-preview-badge">v0.3.0 · 公开模拟演示</span><span className="text-xs font-semibold text-[#6D7A74]">Local pilot available separately</span></div><p className="section-kicker mt-6">Next-Dollar Gate · 新品投前决策引擎</p><h1 className="enterprise-hero-title mt-4 max-w-4xl">下一笔新品费用投入前，<span>先判断最缺什么证据。</span></h1><p className="mt-6 max-w-2xl text-base leading-8 text-[#56665E] sm:text-lg">帮助新品负责人在打样、备货、投放或渠道费用发生前，看清高风险假设，并判断应该继续、补证还是停止。</p><p className="audience-line"><Target className="h-4 w-4" /><span>面向需要反复开发消费新品，并对下一笔费用负责的新品负责人、产品负责人和中小企业经营者。</span></p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="/workspace" className="primary-action px-5 py-3">体验完整模拟流程<ArrowRight className="h-4 w-4" /></Link><Link href="/decision" className="secondary-action px-5 py-3">查看模拟决策结果</Link></div><p className="mt-4 max-w-2xl text-xs leading-5 text-[#7A867F]">公开站使用固定模拟数据说明方法；v0.3.0真实闭环只在本地集成构建运行，不提供云端后端。</p></div>
