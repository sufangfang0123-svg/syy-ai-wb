...
const mechanism = [
  ...
  [FlaskConical, "设计最低成本验证", "由负责人锁定一个主变量，并预设通过、补证和停止阈值。"],
...
export default function ProductSite() {
  ...
  return <div>
    <section className="hero-section"><div className="mx-auto grid max-w-[1360px] items-center gap-10 px-4 py-14 sm:px-6 md:py-20 lg:grid-cols-[1.08fr_.92fr] lg:px-8 lg:py-24">
      <div><div className="flex flex-wrap items-center gap-3"><span className="enterprise-preview-badge">v0.4.0 · 公开互动演示</span><span className="text-xs font-semibold text-[#315C46]">本地封闭试点能力 · 待企业材料验证</span></div><p className="section-kicker mt-6">Evolution Lab · Next-Dollar Gate</p><h1 className="enterprise-hero-title mt-4 max-w-4xl">下一笔新品费用投入前，<span>明确下一项待补证据。</span></h1><p className="mt-6 max-w-2xl text-base leading-8 text-[#56665E] sm:text-lg">新品投前决策与证据验证工作台，帮助负责人在打样、备货、投放或渠道费用发生前，看清高风险假设，并判断应该继续、补证还是停止。</p><p className="audience-line"><Target className="h-4 w-4" /><span>面向需要反复开发消费新品，并对下一笔费用负责的新品负责人、产品负责人和中小企业经营者。</span></p><div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><Link href="/workspace" className="primary-action px-5 py-3">体验完整模拟流程<ArrowRight className="h-4 w-4" /></Link><Link href="/judge-kit" className="secondary-action px-5 py-3">评委三分钟入口</Link><Link href="/enterprise-demo" className="secondary-action px-5 py-3">查看企业闭环演示</Link><Link href="/proof" className="secondary-action px-5 py-3">查看本地闭环系统证明</Link><Link href="/pilot" className="secondary-action px-5 py-3">查看企业试点说明</Link></div><p className="mt-4 max-w-2xl text-xs leading-5 text-[#6D7973]">公开站是模拟演示，只运行固定夹具；系统验收证明来自 local_integrated 本地真实项目闭环。当前不是云生产系统，GitHub Pages不运行FastAPI或SQLite。</p></div>
    ...
    <section id="mechanism" className="page-frame"><div className="section-intro"><div><p className="section-kicker">Decision output</p><h2>企业访客最终会看懂什么</h2><p>从证据到行动，用一条清楚的因果链解释“为什么现在要补证”。</p></div><div className="outcome-strip">{["当前 Gate 状态", "关键证据与缺口", "最高风险假设", "已记录的验证方案"].map((item) => <span key={item}><CheckCircle2 className="h-4 w-4" />{item}</span>)}</div></div><div className="mt-12 max-w-3xl"><p className="section-kicker">One engine, one decision moment</p><h2 className="mt-3 text-3xl font-semibold tracking-tight">不是生成更多创意，而是明确下一项待补证据</h2></div><div className="mt-8 grid gap-4 md:grid-cols-5">{mechanism.map(([Icon, title, body], index) => <article key={title} className="mechanism-card"><span>{String(index + 1).padStart(2, "0")}</span><Icon className="mt-8 h-6 w-6 text-[#5B8C5A]" /><h3>{title}</h3><p>{body}</p></article>)}</div></section>
    ...
    <section id="case" className="border-y border-[#DFE6E9] bg-white/60"><div className="page-frame"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="section-kicker">First built-in industry validation pack</p><h2 className="mt-3 text-3xl font-semibold">棉生万物 · 棉品行业验证包</h2><p className="page-description">以下项目、证据、指标和结论均为固定演示夹具，只用于说明信息结构，不是客户案例。</p></div><span className="simulation-chip">模拟案例 · 非客户成果</span></div><div className="mt-8 grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
      <article className="panel-surface"><p className="section-kicker">Shared demo chain</p><h3 className="mt-2 text-xl font-semibold">{project.name}</h3><div className="mt-6 space-y-3">{[["模拟项目ID", project.id], ["模拟下一投入", `${project.nextAction} · ¥${project.nextInvestmentAmount?.toLocaleString("zh-CN")}`], ["模拟证据", decision.evidenceIds.join("、")], ["模拟验证", `固定夹具 · ${state.tests[0].primaryVariable}`], ["模拟Gate", "仅展示流程，不构成真实判断"]].map(([label, value]) => <div key={label} className="fact-row"><span>{label}</span><strong>{value}</strong></div>)}</div></article>
    ...
    <section id="versions" className="page-frame"><div className="text-center"><p className="section-kicker">Implementation status</p><h2 className="mt-3 text-3xl font-semibold">当前能力与明确边界</h2></div><div className="mx-auto mt-8 grid max-w-5xl gap-5 md:grid-cols-3">
      ...
      <StatusCard icon={CheckCircle2} title="本地集成构建" status="v0.4.0本地封闭试点能力" items={["FastAPI与SQLite", "企业材料与五维Gate", "本机数据持久化"]} />
