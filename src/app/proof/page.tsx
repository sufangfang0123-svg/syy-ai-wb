...
const innovations = [
  ...
  [FlaskConical, "推荐下一项证据", "把结论缺口转成最低成本、可设阈值的验证动作。"],
...
export default function ProofPage() {
  return <div>
    ...
    <section className="page-frame delivery-section"><div className="delivery-heading"><p className="section-kicker">Deterministic gate</p><h2>五维Gate与人工责任边界</h2><p>NEED、COMMERCIAL、PRODUCT、SUPPLY、COMPLIANCE 由 <code>NDG_GATE_V0.3.0</code> 确定性规则计算；人工负责人作最终决定。当前版本没有AI模型参与Gate。</p></div><div className="gate-dimensions">{["NEED 需求","COMMERCIAL 商业","PRODUCT 产品","SUPPLY 供应","COMPLIANCE 合规"].map(item=><span key={item}>{item}</span>)}</div><div className="delivery-grid three mt-8">{innovations.map(([Icon,title,body])=><ProofCard key={title} icon={Icon} title={title} body={body}/>)}</div></section>
