...
export default function ProofPage() {
  return <div>
    ...
    <section className="page-frame delivery-section"><div className="delivery-heading"><p className="section-kicker">Verified capability</p><h2>当前工程实现与离线验收证据</h2><p>Evidence Copilot真实Provider成功UAT尚未取得；本页不把fixture回归写成真实AI验收。</p></div><div className="delivery-grid two"><article className="delivery-card"><BadgeCheck/><h3>本地真实闭环</h3><ul><li>FastAPI＋SQLite单企业本地持久化</li><li>授权材料、候选Evidence与人工复核工程链路</li><li>五维假设、阈值验证、Gate与人工Decision</li><li>stale、下一轮、审计与JSON导出</li></ul></article><article className="delivery-card"><ClipboardCheck/><h3>自动化与部署</h3><ul><li>后端、AI专项、单元、public/local E2E</li><li>public_demo与local_integrated双构建</li><li>公开产物敏感文件与UAT哈希扫描</li><li><a href={`${repo}/actions/workflows/ci.yml`} target="_blank" rel="noopener noreferrer">查看持续集成</a> · <a href={`${repo}/actions/workflows/deploy.yml`} target="_blank" rel="noopener noreferrer">查看Pages部署</a></li></ul></article></div><div className="boundary-panel"><ShieldCheck/><div><strong>公开与本地严格分离</strong><p>GitHub Pages只运行 public_demo：固定模拟夹具、方法说明和系统验收材料。真实项目、FastAPI、SQLite、provider调用与私有附件只在企业本机的 local_integrated 运行。</p></div></div></section>

    <section className="page-frame delivery-final"><div className="delivery-cta"><div><p className="section-kicker">Known limits</p><h2>当前是单企业、本地、单用户的封闭试点</h2><p>没有云后端、账号、RBAC、多租户、企业审批或全网采集；Evidence Copilot只生成待人工复核候选，不得用于自动作出生产、投资、合规或经营决策。</p></div><Link href="/pilot" className="primary-action px-5 py-3">查看试点条件<ArrowRight className="h-4 w-4" /></Link></div></section>
  </div>;
}
