...
const stages = [
  ...
  {href:"/launch",label:"数字实验",step:"04",icon:FlaskConical,paths:["/launch"]},
...
export function LocalProductWorkbenchShell({children}:{children:ReactNode}) {
  ...
  const concept=localBundle?.concepts.find((item)=>item.status==="selected")??localBundle?.concepts.at(-1);
