...
export function ProductWorkbenchProvider({ children }: { children: ReactNode }) {
  ...
  const reviewScenario=useCallback((id:string,status:ScenarioReview["status"],owner:string,note:string)=>mutate("复核数字情景",id,`${status} · ${owner||"待认领"}`,(draft)=>{const scenario=scenarios.find((item)=>item.id===id);if(!scenario)return;const existing=draft.scenarioReviews.find((item)=>item.scenarioId===id);const payload={scenarioId:id,conceptId:scenario.conceptId,assumptionId:scenario.assumptionId,status,owner,note,stale:false,updatedAt:new Date().toLocaleString("zh-CN",{hour12:false})};if(existing)Object.assign(existing,payload);else draft.scenarioReviews.push(payload);}),[mutate,scenarios]);
