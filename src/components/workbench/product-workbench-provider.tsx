...
import { loadPublicFixtureState, savePublicFixtureState } from "@/lib/public-workbench-adapter";
...
export function ProductWorkbenchProvider({ children }: { children: ReactNode }) {
  ...
  const reviewOpportunity=useCallback((id:string,status:WorkflowStatus,note:string)=>mutate("复核机会",id,`${status} · ${note||"未填写补充说明"}`,(draft)=>{draft.selectedOpportunityId=id;draft.opportunityStatus[id]=status;draft.opportunityNotes[id]=note;}),[mutate]);
  const selectConcept=useCallback((id:string,reason:string)=>mutate("选择产品概念",id,reason,(draft)=>{draft.selectedConceptId=id;draft.concepts.forEach((item)=>{item.status=item.id===id?"selected":item.status==="selected"?"candidate":item.status;});const selected=draft.concepts.find((item)=>item.id===id);if(selected)selected.selectionReason=reason;draft.contentAssets.forEach((item)=>{if(item.conceptId!==id){item.conceptId=id;item.stale=true;}});}),[mutate]);
  ...
  const reviewScenario=useCallback((id:string,status:ScenarioReview["status"],owner:string,note:string)=>{const scenario=scenarios.find((item)=>item.id===id);if(!scenario)return;const evidenceIds=state.concepts.find((item)=>item.id===scenario.conceptId)?.evidenceIds??[];mutate("复核数字情景",id,`${status} · ${owner||"待认领"} · ${note||"未填写理由"} · Evidence ${evidenceIds.join("/")||"无"}`,(draft)=>{const existing=draft.scenarioReviews.find((item)=>item.scenarioId===id);const payload={scenarioId:id,conceptId:scenario.conceptId,assumptionId:scenario.assumptionId,status,owner,note,evidenceIds,stale:false,updatedAt:new Date().toLocaleString("zh-CN",{hour12:false})};if(existing)Object.assign(existing,payload);else draft.scenarioReviews.push(payload);});},[mutate,scenarios,state.concepts]);
  ...
  const reviewContent=useCallback((id:string,status:ReviewStatus,note:string)=>mutate("审核内容版本",id,`${status} · ${note||"未填写审核意见"}`,(draft)=>{const item=draft.contentAssets.find((asset)=>asset.id===id);if(item){item.reviewStatus=status;item.reviewNote=note;}}),[mutate]);
  ...
  const value=useMemo<ProductWorkbenchContextValue>(()=>({mode,state,selectedConcept,scenarios,localProjects,localCategoryPacks,localBundle,localLoading,localError,refreshLocal,selectLocalProject,createLocalProject,mutateLocal,uploadLocal,reviewOpportunity,selectConcept,updateConcept,lockConcept,reviewScenario,updateContent,reviewContent,addFeedback,addNextRoundItem}),[mode,state,selectedConcept,scenarios,localProjects,localCategoryPacks,localBundle,localLoading,localError,refreshLocal,selectLocalProject,createLocalProject,mutateLocal,uploadLocal,reviewOpportunity,selectConcept,updateConcept,lockConcept,reviewScenario,updateContent,reviewContent,addFeedback,addNextRoundItem]);
