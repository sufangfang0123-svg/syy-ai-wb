...
export function SiteShell({ children }: { children: ReactNode }) {
  ...
  if (marketing) {
    return <div className="min-h-screen bg-[#FAF8F5] text-[#2D3436]">
      <header className="site-header"><div className="mx-auto flex h-[72px] max-w-[1500px] items-center gap-5 px-5 lg:px-10">
        <Brand />
        ...
        <Link href="/workspace" className="primary-action ml-2">{config.isPublicDemo ? "进入模拟研究实验室" : "打开本地集成工作区"}</Link>
      </div></header>
      <main>{children}</main>
      <footer className="border-t border-[#DFE6E9] bg-[#F3F1EC] px-5 py-6 text-center text-xs leading-6 text-[#636E72]">当前公开页面是展示站与独立模拟研究实验室，不执行真实采集、真实AI分析或真实项目决策。</footer>
    </div>;
  }
  ...
  return <div className="min-h-screen bg-[#FAF8F5] text-[#2D3436]" data-build-profile={config.buildProfile} data-active-space={activeSpace}>
    ...
    <footer className="border-t border-[#DFE6E9] bg-[#F3F1EC] px-4 py-3 text-center text-xs text-[#636E72]">{realBoundaryOpen ? "本地服务健康检查已通过，但1A-0不提供真实项目数据操作。" : "独立模拟研究实验室：所有记录均为演示数据，只保存在当前浏览器。"}</footer>
