...
浏览器控制通道不可用，无法可靠读取或编辑飞书原文。用户已导出原始DOCX，原件保持不变，并已生成独立修订终稿与逐章复制映射；飞书仍需有权限的人工账号完成一次导入和终审。

180秒完整版与60秒精华版均已完成本地成片、解码校验和画面复核。最终收口结果以[比赛收口QA](competition-closeout-qa-v0.4.0.md)为准。
